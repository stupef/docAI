...
/**
 * 向量存储（默认实现）：HNSW 内存索引 + Redis 持久化。
 * <p>
 * 由 {@code rag.vector.backend=redis}（默认）激活。详见 {@link VectorStore}。
 */
@Component
@ConditionalOnProperty(name = "rag.vector.backend", havingValue = "redis", matchIfMissing = true)
public class HnswRedisVectorStore implements VectorStore {

    private static final Logger log = LoggerFactory.getLogger(HnswRedisVectorStore.class);
    ...
    public void store(String id, float[] vector, String content, Map<String, Object> metadata) {
        ...
        try {
            ...
            // 原文随向量写入 segment:，BM25 / getSegmentContent 直接从 Redis 取（Redis 后端不依赖 Qdrant）
            if (content != null) {
                redisTemplate.opsForValue().set(segmentKey, content);
            }
    ...
    public String getContent(String id) {
        try {
            Object content = redisTemplate.opsForValue().get(SEGMENT_PREFIX + id);
            return content == null ? null : content.toString();
        } catch (Exception e) {
            log.warn("获取 chunk 原文失败", e);
            return null;
        }
    }
    ...
    public Map<String, Map<String, Object>> scanChunks(Map<String, Object> filters) {
        ...
        List<String> keys = scanKeys(METADATA_PREFIX + "*", 1000);
        if (keys == null) {
            return result;
        }
        for (String key : keys) {
            String id = key.substring(METADATA_PREFIX.length());
            Map<String, Object> meta = getMetadata(id);
            if (!matchesFilters(meta, filters)) {
                continue;
            }
            // 附带原文（segment:），BM25 打分直接使用，免去检索期再读 Redis
            Object content = redisTemplate.opsForValue().get(SEGMENT_PREFIX + id);
            if (content != null) {
