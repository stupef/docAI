...
├── config/
│   ├── security/                       # 安全配置
│   │   ├── BaseSecurityConfig          # 基础安全配置类，提供通用的安全设置，供各服务继承使用
│   │   ├── JwtAuthenticationFilter     # JWT认证过滤器，验证请求中的JWT令牌并设置认证上下文
│   ├── FeignGlobalConfig               # Feign全局配置（超时/拦截）
│   ├── RedisConfig                     # Redis序列化配置
│   ├── SwaggerConfig                   # 配置API文档和Swagger UI
│   ├── WebMvcConfig                    # Web配置（跨域/拦截器）
├── constant/                           # 定义常量
│   ├── CommonConstant                  # 通用常量（分页/正则）
│   ├── ErrorCodeEnum                   # 错误码枚举
│   ├── RedisKeyEnum                    # Redis键名枚举
├── exception/                          # 异常处理
│   ├── BusinessException               # 业务异常，保存异常信息
│   ├── TokenException                  # 令牌异常，保存异常信息
│   ├── GlobalExceptionHandler          # 决定给前端展示什么样的异常，打印异常给后端
├── model/                              # 数据模型
│   ├── dto/                            # 进门时的数据
│   │   ├── PageDTO
│   │   ├── UserDTO
...
│   ├── Result                          # 全局统一返回结果
├── utils/                              # 可复用的工具
│   ├── DataUtils                       # 日期时间工具（格式化/解析/转换/加减）
│   ├── FileUtils                       # 文件工具
│   ├── JwtUtils                        # JWT工具（生成/解析/验证）
│   ├── RedisUtils                      # Redis工具（增删改查/过期）
└── ├── ValidateUtils                   # 参数校验工具（非空/格式）
```
```
### 用户服务模块
user-service/
